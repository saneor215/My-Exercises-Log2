        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl p-6 ring-1 ring-white/10 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-white">{routineToEdit ? t('edit') : t('create_new_plan')}</h2>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-700"><XIcon className="w-6 h-6"/></button>
                </div>

                <div className="mb-4">
                    <label htmlFor="routine-name" className="block text-sm font-medium text-gray-300 mb-2">{t('plan_selected')}</label>
                    <input id="routine-name" type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Push Day" className="w-full bg-gray-700 text-white px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                
                <div className="mb-4">
                    <h3 className="text-lg font-bold text-gray-200 mb-2">{t('add_new_exercise')}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <select value={selectedPart} onChange={e => setSelectedPart(e.target.value)} className="md:col-span-1 bg-gray-700 text-white px-3 py-2 rounded-lg">
                            {bodyParts.map(p => <option key={p.id} value={p.id}>{t(`part_${p.id}` as any) || p.name}</option>)}
                        </select>
                        ...
                        <button onClick={handleAddExercise} disabled={!selectedExercise} className="md:col-span-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-lg disabled:bg-gray-600">{t('add')}</button>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto pr-2 -mr-2 space-y-2 border-t border-gray-700 pt-4">
                    <h3 className="text-lg font-bold text-gray-200 mb-2">{t('exercises_list')}</h3>
                        ...
                        return (
                            <div key={index} className="bg-gray-700/50 p-2 rounded-lg flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-400">{t(`part_${re.partId}` as any) || details?.partName || re.partId}</p>
                                </div>
                    ...
                    }) : <p className="text-gray-500 text-center py-4">{t('no_logs_title')}</p>}
                </div>
                
                <div className="flex justify-end gap-4 pt-4 mt-4 border-t border-gray-700">
                    <button onClick={onClose} className="px-6 py-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white font-semibold">{t('cancel')}</button>
                    <button onClick={handleSave} className="px-6 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-semibold">{t('save')}</button>
                </div>
            </div>
        </div>
    );
