  ...
  return (
    <div className="bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg ring-1 ring-white/10">
      <div className="flex items-center justify-between mb-6">
        <button onClick={handleNextMonth} className="p-2 rounded-full hover:bg-gray-700 transition-colors text-gray-300 hover:text-white"><ChevronRightIcon className="w-6 h-6" /></button>
        ...
        <button onClick={handlePrevMonth} className="p-2 rounded-full hover:bg-gray-700 transition-colors text-gray-300 hover:text-white"><ChevronLeftIcon className="w-6 h-6" /></button>
      </div>
      <div className="grid grid-cols-7 gap-1 text-center text-sm">
        ...
        {calendarDays.map((day, index) => {
          ...
          const dotColorClass = workoutDaysInMonth.get(dayStr);
          ...
          if (isSelected) {
            dayClasses = "bg-blue-600 text-white font-bold scale-110 shadow-lg ring-2 ring-blue-400";
          ...
          return (
            <div key={index} className="flex justify-center items-center p-1">
                ...
                >
                    {day.getDate()}
                    {dotColorClass && !isSelected && (
                      <span className={`absolute bottom-1.5 w-1.5 h-1.5 rounded-full ${dotColorClass}`}></span>
                    )}
                </button>
            </div>
          );
